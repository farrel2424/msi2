...
Handles all interactions with the Motorsights EPC API.
Updated to support dynamic bearer token generation via SSO and flat
batch creation for Engine / Transmission partbooks.
...
class MotorsightsEPCClient:
    ...
    def create_category(self, data: Dict) -> Tuple[bool, Optional[Dict], bool]:
        """
        Create new category with optional type categories.

        Returns:
            Tuple of (success, response_data, was_skipped)
            was_skipped=True means the category already existed (409) and was
            skipped gracefully — does NOT count as an error.
        """
    ...
    def batch_create_type_categories_and_categories(
        ...
        Used for the 3-level hierarchy:
            Master Category → Category → Type Category (subtype)

        409 conflicts (duplicate category names) are skipped gracefully and
        counted separately — they do NOT cause the overall batch to fail.

        Args:
            catalog_data:            Dict with "categories" list. Each item has
                                     category_name_en/cn and a "data_type" list.
            master_category_id:      UUID of the master category.
            master_category_name_en: Human-readable name (e.g. "Cabin & Chassis").
        ...
        for pdf_category in catalog_data.get('categories', []):
            ...
            if success and was_skipped:
                ...
                self.logger.info(
                    f"Skipped existing category "
                    f"'{pdf_category.get('category_name_en', '')}'"
                )
            elif success:
    ...
    def batch_create_flat_categories(
        ...
        Used for the 2-level hierarchy:
            Master Category → Category   (no subtypes)

        The API create_category endpoint is reused but data_type is sent as [].
        409 conflicts (duplicate names) are skipped gracefully.
        ...
        Args:
            catalog_data:            Dict with "categories" list. Each item has
                                     ...
                                     category_description.
            master_category_id:      UUID of the master category.
            master_category_name_en: Human-readable name (e.g. "Engine").
        ...
        results = {
            'categories_created': [],
            'categories_skipped': [],       # duplicates
            'type_categories_created': [],  # always empty for flat types
            'errors': []
        }
        ...
        for cat in catalog_data.get('categories', []):
            category_request = {
                ...
                "data_type": []     # flat — no subtypes
            }

            self.logger.debug(f"Creating flat category: {category_request}")
            ...
            if success and was_skipped:
                ...
                self.logger.info(
                    f"Skipped existing category "
                    f"'{cat.get('category_name_en', '')}'"
                )
            elif success:
                results['categories_created'].append(cat_response.get('data', {}))
                self.logger.info(
                    f"Created category '{cat.get('category_name_en', '')}'"
                )
            else:
                error_detail = cat_response.get('error') if cat_response else 'Unknown error'
                results['errors'].append({
                ...
                self.logger.error(
                    ...
                    f"'{cat.get('category_name_en', '')}': {error_detail}"
                )

        overall_success = len(results['errors']) == 0

        self.logger.info(
            f"Flat batch complete: "
            f"{len(results['categories_created'])} created, "
            f"{len(results['categories_skipped'])} skipped (duplicates), "
            f"{len(results['errors'])} errors"
        )
